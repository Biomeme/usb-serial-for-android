...
public class FtdiSerialDriver implements UsbSerialDriver {
    ...
    public FtdiSerialDriver(UsbDevice device) {
        mDevice = device;
        mPort = new FtdiSerialPort(mDevice, 1);
    }
    @Override
    ...
    private class FtdiSerialPort extends CommonUsbSerialPort {
        ...
        public void reset() throws IOException {
            int index = mDevice.getInterface(mInterface).getId() + 1;
            int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE, SIO_RESET_REQUEST,
                    SIO_RESET_SIO, index, null, 0, USB_WRITE_TIMEOUT_MILLIS);
            if (result != 0) {
        ...
        public int read(byte[] dest, int timeoutMillis) throws IOException {
            final UsbEndpoint endpoint = mDevice.getInterface(mInterface).getEndpoint(0);
        ...
        public int write(byte[] src, int timeoutMillis) throws IOException {
            final UsbEndpoint endpoint = mDevice.getInterface(mInterface).getEndpoint(1);
            int offset = 0;
        ...
        private int setBaudRate(int baudRate) throws IOException {
            ...
            long value = vals[2];
            int index = mDevice.getInterface(mInterface).getId() + 1 + (int) vals[1];

            int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE,
                    SIO_SET_BAUD_RATE_REQUEST, (int) value, index,
        ...
        public void setParameters(int baudRate, int dataBits, int stopBits, int parity)
            ...
            int index = mDevice.getInterface(mInterface).getId() + 1;
            int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE,
                    SIO_SET_DATA_REQUEST, config, index,
                    null, 0, USB_WRITE_TIMEOUT_MILLIS);
            if (result != 0) {
        ...
        public boolean purgeHwBuffers(boolean purgeReadBuffers, boolean purgeWriteBuffers) throws IOException {
            ...
            if (purgeReadBuffers) {
                int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE, SIO_RESET_REQUEST,
                        SIO_RESET_PURGE_RX, index, null, 0, USB_WRITE_TIMEOUT_MILLIS);
                if (result != 0) {
            ...
            if (purgeWriteBuffers) {
                int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE, SIO_RESET_REQUEST,
                        SIO_RESET_PURGE_TX, index, null, 0, USB_WRITE_TIMEOUT_MILLIS);
                if (result != 0) {
                    throw new IOException("Flushing RX failed: result=" + result);
                }
            }
